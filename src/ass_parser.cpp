...
public:
	HeaderToProperty()
		: fields({{"Automation Scripts", &ProjectProperties::automation_scripts},
...
AssParser::AssParser(AssFile *target, int version)
	: property_handler(std::make_unique<HeaderToProperty>()), target(target), version(version), state(&AssParser::ParseScriptInfoLine)
{
}
