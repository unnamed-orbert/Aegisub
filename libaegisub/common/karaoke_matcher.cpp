											  ...
											  std::string_view dest_string)
{
	...
	while (!src.done())
	{
		...
		{
			if (!dst.current_to_end().starts_with(kana))
				continue;
	...
	for (size_t lookahead = 0; lookahead < dst_lookahead_max; ++lookahead)
	{
		...
		for (auto const &syl : source_strings)
		{
			...
			if (is_whitespace(syl))
				continue;
			if (++src_lookahead_pos == 1)
				continue;
			if (src_lookahead_pos > src_lookahead_max)
				break;

			std::string lsyl(syl);
			boost::to_lower(lsyl);
			if (!(syl.starts_with(cur) || util::any_of(translit, [&](auto s)
													   { return lsyl.starts_with(s); })))
				continue;
